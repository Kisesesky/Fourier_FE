// app/(workspace)/workspace/[teamId]/[projectId]/calendar/_components/CalendarTimelineView.tsx
'use client';
...
export function CalendarTimelineView({
  ...
  const cellWidth = TIMELINE_DAY_CELL_MIN_WIDTH;
  ...
  const monthHeaderCells = useMemo(() => {
    ...
    return monthSegments.map((segment, index) => {
      const cell = (
        <div
          key={segment.key}
          className={`flex items-center justify-center border-r border-border text-xs font-semibold text-foreground ${TIMELINE_MONTH_COLORS[index % TIMELINE_MONTH_COLORS.length]}`}
                    ...
                    >
                      ...
                      {group.tasks.map((task, index) => (
                        <TimelineTaskBar
                          ...
                          totalDays={totalDays}
                          hint={`${formatTimelineRange(task.start, task.end, task.allDay)} · ${task.calendarName}`}
